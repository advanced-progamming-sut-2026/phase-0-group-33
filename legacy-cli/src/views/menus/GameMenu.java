...
public class GameMenu implements AppMenu {
        ...
        private void registerBattleCommands(GameController controller) {
                router.add(GameCommands.ADVANCE_TIME.pattern,
                                matcher -> controller.handleAdvanceTime(Integer.parseInt(matcher.group("count"))))
                                .add(GameCommands.COLLECT_SUN.pattern, matcher -> controller.handleCollectSun(
                                                Integer.parseInt(matcher.group("x")),
                                                Integer.parseInt(matcher.group("y"))))
                                .add(GameCommands.CHEAT_ADD_SUN.pattern,
                                                matcher -> controller.handleCheatAddSun(
                                ...
                                .add(GameCommands.CHEAT_SPAWN_ZOMBIE.pattern,
                                                matcher -> controller.handleCheatSpawnZombie(
                                                                matcher.group("type"),
                                                                Integer.parseInt(matcher.group("x")),
                                                                Integer.parseInt(matcher.group("y"))));
                registerMinigameCommands(controller);
        }

        private void registerMinigameCommands(GameController controller) {
                router.add(GameCommands.BREAK_VASE.pattern, matcher -> controller.handleBreakVase(
                                Integer.parseInt(matcher.group("x")), Integer.parseInt(matcher.group("y"))))
                                .add(GameCommands.PLACE_ZOMBIE.pattern, matcher -> controller.handlePlaceZombie(
                                                matcher.group("type"),
                                                Integer.parseInt(matcher.group("x")),
                                                Integer.parseInt(matcher.group("y"))))
                                .add(GameCommands.SWAP_PLANTS.pattern, matcher -> controller.handleSwap(
                                ...
                                .add(GameCommands.BEGHOULED_UPGRADE.pattern,
                                                matcher -> controller.handleBeghouledUpgrade(matcher.group("type")));
        }
